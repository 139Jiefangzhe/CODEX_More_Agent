...
-- Logical pool current state (single row per park)
...
-- Allocation ledger (one session can enter/exit multiple times)
...
-- Source event process log for idempotency and audit
...
-- Daily report counters (report pool only)
